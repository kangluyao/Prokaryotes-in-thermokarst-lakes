...
##plot the total community composition
sample_data(meta.com.cla)$ALL <- c(rep('All', 320)) # create a new variable
##plot
microbiome::plot_composition(meta.com.cla, 
                             average_by = "ALL", 
                             otu.sort = "abundance") +
...
##plot the community composition for TP and PA
...
carbon_fun_plot
library(grid)
vie <- viewport(width=0.5, height=0.45, x=0.25, y=0.75)
print(pie_for_fun, vp = vie)
...
vars <- colnames(t2$res_spe_func_perc[ ,colnames(t2$res_spe_func_perc) %in% c(t2$func_group_list$`N-cycle`)])
library(lme4)
library(lmerTest)
mode <- lapply(vars, function(x) {
...
tp_total_alpha_div <- tp_total_alpha_div[ , c("Chao1", 'Shannon', 'Simpson')]
tp_carbon_alpha_div <- estimate_richness(tp_C_phylo, measures = c("Chao1", 'Shannon', 'Simpson'))
tp_carbon_alpha_div <- tp_carbon_alpha_div[ , c("Chao1", 'Shannon', 'Simpson')]
...
tp_alpha_div_plot_data <-  tp_alpha_div %>% 
  dplyr::group_by(group, Site, Sitegroup1) %>%
  dplyr::summarise(across(, mean, na.rm = TRUE)) %>%
  tidyr::gather(diversity, value, -c('group', 'Site', 'Sitegroup1', 'latitude', 'longitude')) %>%
  dplyr::mutate(group = factor(group, levels = c('Total comunity', 'Carbon cycling comunity')))
...
for(i in 1:30){
  ...
  adj.r.squared <- rse_sum$adj.r.squared # obtain the adjust r squared
  multicollinearity <- any(car::vif(A2@objects[[i]]) > 2) # check the multicollinearity
  tmp <- data.frame(adj.r.squared, multicollinearity)
...
## (partial) mantel test for each variable for Tibetan Plateau
library(ggcor)
## mantel test function
mantel.fun <- function(phylo) {
  env.table <- data.frame(sample_data(phylo))
